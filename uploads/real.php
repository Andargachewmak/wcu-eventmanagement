...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['event_image']) && $_FILES['event_image']['error'] == 0) {
        ...
        if ($_FILES["event_image"]["size"] > 2000000) {
            $message .= "Sorry, your file is too large. Maximum allowed size is 2MB.<br>";
            $uploadOk = 0;
        }
    ...
    $sql = "INSERT INTO events (title, description, event_date, event_time, location, pic, created_by, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    // Prepare the statement
    if ($stmt = $conn->prepare($sql)) {
        // Bind the parameters
        $stmt->bind_param("ssssssis", $event_title, $event_description, $date, $time, $location, $pic, $userId, $current_date);
            ...
            <div class="form-container">
                ...
                <form method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="btn-container">
                        <input type="submit" value="Create Event" class="btn-create">
                        <a class="btn-cancel" href="Acadamic_Dashboard.php">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
        <div class="card">
            <div class="form-container">
                ...
                <table border="1">
                    ...
                    $sql = "SELECT title, description, event_date, event_time, location, pic, created_by, created_at FROM events ORDER BY created_at DESC";
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        <td>" . $row["location"] . "</td>
                        <td><img src='../upload/" . $row["pic"] . "' alt='Event Image'></td>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare the SQL query to insert the event data into the database
    $sql = "INSERT INTO events (title, description, event_date, event_time, location, created_by, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        <div class="card">
            <div class="form-container">
                ...
                <table border="1">
                    ...
                    // SQL query to select events
                    $sql = "SELECT title, description, event_date, event_time, location, created_by, created_at FROM events ORDER BY created_at DESC";
                    $result = $conn->query($sql);
